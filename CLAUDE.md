...
Two sections are per-brief rather than per-registry: Historical Research takes its dates and respondent base from the study that matched, and the live sources (web research, Google Trends) are dated with the run date via the `{run_date}` token. Sections a brief did not render carry no line at all. Because the deck's appendix reaches clients, source strings name systems ("the ad deliveries central benchmarking sheet"), never repository paths.
