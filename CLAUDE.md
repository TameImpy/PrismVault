...
Segments carry a `match_reason` built from the same keyword weights that ranked them — the terms are ordered by their real contribution (a rare term hitting the segment's name outranks a common one in the description) and capped at three so the line stays one line. A segment that arrived on the category rung of the match ladder says `"Matched on category: Gardening"` instead, because a category match is a broader, weaker claim and reading as much is the point. The value of naming terms is diagnostic: a planner seeing `"Matched on: hearing, over 65s"` on a hearing-aids brief can tell for themselves that the match is demographic rather than subject-led — the report that prompted #163.
