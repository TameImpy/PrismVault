...
**Insight tiles:** the tile mirrors the product tile — `[INSIGHT_n]` is the 40pt ExtraBold hero line and carries the *figure*; `[INSIGHT_n_STAT]` is the 26pt ExtraLight line beneath it and carries the supporting phrase. (The marker names read the other way round; the layout is what the code follows.) The supporting phrase is the tightest tile in the deck: two lines of 26pt in 3.54", about six words. All three tiles take the third tile's width — the Prism logo sits under it — because three tiles drawn identically have to wrap identically.

**Tile budgets (#162):** every marker has a width and a line count it may grow to, held in `TILE_BUDGETS` in `src/tile_fit.py` and enforced on every value before it is written. The budgets are a *reading* of this file, not a preference — `tests/test_tile_fit.py` asserts each one uses the template's own typeface and point size, equals its box's wrapping width exactly, and does not overlap another tile, the slide edge or the logo artwork when full. Move a box in the template and that test fails rather than a client-facing deck picking up overlapping text.

The boxes are all `spAutoFit`, so PowerPoint grows a shape downward at layout time to fit whatever text it holds; python-pptx does not, which is why an overflowing value looks fine in the file and lands on the tile below it on screen. Several boxes were also drawn wider than the column they sit in — `[INSIGHT_2_STAT]` was a 5.34" box at x=4.88" on a 10" slide — so text wrapped at the box, outside its tile. `scripts/narrow_overwide_tiles.py` brought those boxes in to their columns and is the record of that edit; it touches `cx` only, never a font, position or colour.

The segment name tile takes four 11pt lines so the catalogue's longest name (95 characters) lands whole, and the format tile takes two so every name in `data/format_recommendations.csv` does — tests assert both, because a truncated product name on a client deck is a defect of its own. A value that genuinely will not fit is cut on a word boundary and marked with `…`; the cut is visible deliberately, since a silently clipped segment name reads as a different segment.
